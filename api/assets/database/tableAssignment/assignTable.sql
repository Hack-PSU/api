...
begin
	declare hackathon_var varchar(45);
    ...
    set tableNumber_param := 
        (select tableID from TABLE_SUPPORT 
            where categoryID = categoryID_param and tableID not in (select tableNumber from TABLE_ASSIGNMENTS where hackathon = hackathon_var)
