...
const { verifyACL } = require('../../services/auth');
 ...
 * @apiParam {String} userUid The uid of the user checking out the item
...
router.post('/', verifyACL(2), (req, res, next) => {
  if (!req.body || !req.body.itemId || !req.body.userUid) {
  ...
  const checkoutInstance = new CheckoutObject(
    Object.assign(
      req.body,
      { checkoutTime: new Date().getTime().toString() },
 ...
 * @apiUse IllegalArgumentError
 */
router.get('/items/availability', verifyACL(2), (req, res, next) => {
  return CheckoutItem.getAllAvailable(req.uow)
    .then(stream => streamHandler(stream, res, next))
    .catch(err => errorHandler500(err, next));
});
