...
def createTravelReimbursment(date, description, price, payee, mailingAddress, receipts):
	#Hard code this to look for check_request_form

	fpath = args.form if args.form is not None else "./CHECK_REQUEST_FORM.pdf"
	packet = BytesIO()
	...
	output.addPage(page)
	if receipts is not None and receipts != "NULL":
	    receipt_array = receipts.split(',')
	    try:
	        for receipt in receipt_array:
	            print("Downloading image")
	            print(receipt)
	            filename, headers = urllib.urlretrieve(receipt, "./receipts/" +
	                    ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(10)) + '.jpg')
	            print("Downloaded file as %s" % filename)
	            pdf = FPDF(unit='in')
	            pdf.add_page()
	            pdf.image(filename, h=11, w=9.5)
	            pdf.output(filename + ".pdf")
	            receipt_page_reader = PdfFileReader(filename + ".pdf")
	            output.addPage(receipt_page_reader.getPage(1))
...
parser.add_argument('form', metavar="-F", type=str, help="The path to the check request form")
...
with open(args.input, "rt") as f:
	...
	for row in mycsv:
		...
		receipts = row[3] if len(row) > 3 else None
		createTravelReimbursment(date, description, price, payee, mailingAddress, receipts)
